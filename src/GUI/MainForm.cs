...
namespace Draw
{
	...
	public partial class MainForm : Form
	{
		...
		{
			string name = Microsoft.VisualBasic.Interaction.InputBox("Въведете име:");

			if (name == "")
			{
				MessageBox.Show("The shape must have a name");
				return;
			}
			dialogProcessor.AddRandomRectangle(name);
		...
		void ViewPortMouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			if (pickUpSpeedButton.Checked)
			{
				...
				if (temp != null)
				{
					// Проверяваме дали temp е част от група
					GroupShape parentGroup = dialogProcessor.GetParentGroupOfShape(temp);

					if (Control.ModifierKeys == Keys.Control && parentGroup != null && temp != parentGroup)
					{
						// Разгрупираме фигурата от групата САМО ако не е самата група
						dialogProcessor.UngroupShapeFromGroup(parentGroup, temp);
					...
					{
						// Нормална селекция с или без Ctrl
        ...
        {
			string name = Microsoft.VisualBasic.Interaction.InputBox("Въведете име:");
			dialogProcessor.AddRandomEllipse(name);
        ...
        private void drawStarSpeedButtonClick(object sender, EventArgs e)
        {
			...
			dialogProcessor.AddRandomStar(name);
        ...
        private void drawLineSpeedButtonClick(object sender, EventArgs e)
        {
			...
			dialogProcessor.AddRandomLine(name);
        ...
        private void drawPointSpeedButtonClick(object sender, EventArgs e)
        {
			...
			dialogProcessor.AddRandomPoint(name);
		...
		{
			string color = textBoxNameObject.Text.Trim();
			...
			else
			{
				foreach (Shape shape in dialogProcessor.Selection)
				{
					shape.FillColor = Color.FromName(color);
					viewPort.Invalidate();
				}
			}
		}
